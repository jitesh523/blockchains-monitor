...
def create_risk_score_timeline(dark_mode: bool = True) -> go.Figure:
    ...
    fig.update_layout(
        ...
        title_font_size=16,
        title_x=0.5,
        xaxis=dict(gridcolor=colors["grid"], showgrid=True, gridwidth=1),
        yaxis=dict(gridcolor=colors["grid"], showgrid=True, gridwidth=1, range=[0, 100]),
    ...
    # Add hover template
    fig.update_traces(
        hovertemplate="<b>%{fullData.name}</b><br>" + "Date: %{x}<br>" + "Risk Score: %{y:.1f}<br>" + "<extra></extra>"
    )
...
@performance_monitor
def create_risk_score_timeline_filtered(
    dark_mode: bool = True, protocols: List[str] = None, days: int = 30, height: int = 400
) -> go.Figure:
...
def create_volatility_sentiment_scatter(dark_mode: bool = True) -> go.Figure:
    ...
    fig = px.scatter(
        ...
        labels={"volatility": "Volatility (%)", "sentiment": "Sentiment Score", "tvl": "TVL (M USD)"},
        template=theme_template,
        color_discrete_map={"Low": colors["success"], "Medium": colors["warning"], "High": colors["error"]},
    )
    ...
    fig.update_layout(
        ...
        title_font_size=16,
        title_x=0.5,
        xaxis=dict(gridcolor=colors["grid"], showgrid=True, gridwidth=1, range=[0, 70]),
        yaxis=dict(gridcolor=colors["grid"], showgrid=True, gridwidth=1, range=[-1, 1]),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(l=0, r=0, t=50, b=0),
        height=400,
    )
...
def create_protocol_risk_comparison(dark_mode: bool = True) -> go.Figure:
    ...
    fig.update_layout(
        ...
        title_font_size=16,
        title_x=0.5,
        xaxis=dict(gridcolor=colors["grid"], showgrid=True, gridwidth=1, range=[0, 100]),
...
def render_analytics_dashboard():
    ...
    with control_col1:
        time_range = st.selectbox(
            "Time Range", ["7 Days", "30 Days", "90 Days"], index=1, help="Select the time range for risk analysis"
        )
