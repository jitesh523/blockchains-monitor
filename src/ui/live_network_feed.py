...
class NetworkMonitor:
    ...
    async def get_gas_price(self, network: str, api_key: str = None) -> Dict[str, Any]:
        ...
        try:
            urls = {"ethereum": self.etherscan_url, "polygon": self.polygonscan_url, "arbitrum": self.arbiscan_url}

            if network not in urls:
                return {"error": f"Network {network} not supported"}

            params = {"module": "gastracker", "action": "gasoracle", "apikey": api_key or "YourApiKeyToken"}
        ...
        with st.sidebar.expander(f"🔗 {network.capitalize()}", expanded=True):
            try:
                ...
                if "error" not in stats:
                    st.metric(f"{network.capitalize()} Gas", stats.get("gas_price", "N/A"))
                    st.metric("Block Time", stats.get("block_time", "N/A"))
                    st.metric("TPS", stats.get("tps", "N/A"))
