...
def test_theme_audit():
    ...
    print("-" * 30)
    print(
        f"Dark Mode - Passed: {len(dark_audit['passed'])}, Failed: {len(dark_audit['failed'])}, Warnings: {len(dark_audit['warnings'])}"
    )
    print(
        f"Light Mode - Passed: {len(light_audit['passed'])}, Failed: {len(light_audit['failed'])}, Warnings: {len(light_audit['warnings'])}"
    )
